...
- `../Silent-Storm-Reconstruction/diagnostics/FACE-PARITY.md` — текущие
  ...
  `ITransformer::Load/Generate` для FaceGen ещё не реализован; зелёный тест
  загруженного аниматора не доказывает нативную генерацию FaceGen.
...
генерации; сейчас он ожидаемо падает на x64 `ITransformer::Load` даже для
нейтрального лица. Не скрывайте этот красный результат за первым тестом.
...
Прямой красный gate и отдельная проверка уже готового потока запускаются так:
